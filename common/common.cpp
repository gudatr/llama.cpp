...
{
    std::ofstream ss("log" + std::to_string(i));
    ss << "ARG " << arg;
...
{
    std::ofstream llamainit("llamainit.txt");
    ...
    {
        llamainit << "Loading from file" << std::endl;
        model = llama_load_model_from_file(params.model.c_str(), mparams);
    }

    if (model == NULL)
    {
        llamainit << "Model is NULL, possibly load failure" << std::endl;
    ...
    if (lctx == NULL)
    {
        llamainit << "Failed to create context" << std::endl;
    ...
    if (!params.control_vectors.empty())
    {
        ...
        if (cvec.n_embd == -1)
        {
            llamainit << "Embedding empty but control vectors were supplied" << std::endl;
            llama_free(lctx);
            llama_free_model(model);
            return std::make_tuple(nullptr, nullptr);
        }
        ...
        if (err)
        {
            llamainit << "Failed to apply control vector: " << err << std::endl;
            llama_free(lctx);
            llama_free_model(model);
            return std::make_tuple(nullptr, nullptr);
        }
    ...
    for (unsigned int i = 0; i < params.lora_adapter.size(); ++i)
    {
        ...
        if (err != 0)
        {
            llamainit << "Failed to apply lora adapter" << std::endl;
    ...
    if (params.warmup)
    {
        llamainit << "Warming model up" << std::endl;
        ...
        llamainit << "Warm up finished" << std::endl;
    }
