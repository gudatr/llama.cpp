...
{
    std::ofstream deeper("llamacppdeeper.txt");
    ...
    try
    {
        ...
        {
            deeper << "error loading model architecture: " + std::string(e.what());
        ...
        {
            deeper << "error loading model hyperparameters: " + std::string(e.what());
        ...
        {
            deeper << "error loading model vocabulary: " + std::string(e.what());
        ...
        {
            deeper << "tensorfailure";
            return -2;
        }
    }
    catch (const std::exception &err)
    {
        deeper << "Generic error: " + std::string(err.what());
    ...
    struct llama_model_params params)
{
    ...
    std::ofstream ll("llamacpp.txt");

    ll << "AYO WE LOADING" << std::endl;
    ...
    struct llama_context_params params)
{
    ...
    std::ofstream llamacontext("llamacontext.txt");

    if (!model)
    {
        llamacontext << "Model is null " << std::endl;
    ...
    {
        llamacontext << "n_batch and n_ubatch cannot both be zero " << std::endl;
    ...
    {
        llamacontext << "n_ctx and model->hparams.n_ctx_train cannot both be zero" << std::endl;
    ...
    {
        llamacontext << "V cache quantization requires flash_attn" << std::endl;
    ...
    if (!hparams.vocab_only)
    {
        ...
        if (model->n_gpu_layers > 0)
        {
            ...
            {
                llamacontext << "Failed to initialize Metal backend" << std::endl;
        ...
        if (model->split_mode == LLAMA_SPLIT_MODE_NONE)
        {
            ...
            if (backend == nullptr)
            {
                llamacontext << "Failed to initialize Vulkan backend" << std::endl;
